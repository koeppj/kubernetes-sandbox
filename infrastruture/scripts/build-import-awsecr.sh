...
image="awsecr:1.0.0"
archive_dir=$(mktemp -d -t awsecr-import.XXXXXXXX)
archive_path="$archive_dir/awsecr.tar"
import_log="$archive_dir/import.log"
trap 'rm -f -- "$archive_path" "$import_log"; rmdir -- "$archive_dir"' EXIT

docker build -t "$image" -f "$infrastructure_dir/awsecr.Dockerfile" "$infrastructure_dir"
...
if grep -Eq 'Failed to (import images on|reach)|Could not query for nodes' "$import_log"; then
    echo "AWS ECR image import failed; deployment must not proceed." >&2
    exit 1
fi
